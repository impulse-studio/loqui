...
interface StepWelcomeLocalProps {
  goNext: () => void;
  setFooter: (config: FooterConfig | null) => void;
  onSwitchToCloud: () => void;
}

export default function StepWelcomeLocal({
  goNext,
  setFooter,
  onSwitchToCloud,
}: StepWelcomeLocalProps) {
  ...
  return (
    <div className="text-center">
      ...
      <button
        ...
        disabled={downloading}
        className="mt-6 text-xs text-text-tertiary hover:text-text-secondary underline cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
      >
        I prefer using cloud inference instead
      </button>
    </div>
  );
}
